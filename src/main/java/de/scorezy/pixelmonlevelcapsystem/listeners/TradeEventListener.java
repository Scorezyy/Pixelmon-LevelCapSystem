...
public class TradeEventListener {
    ...
    public void onPixelmonTrade(PixelmonTradeEvent.Pre event) {
        if (!ConfigLoader.getSettingsConfig().isLevelCapPlayerTrades()) return;

        ServerPlayer p1 = (ServerPlayer) event.getPlayer1();
        ServerPlayer p2 = (ServerPlayer) event.getPlayer2();

        if (attemptBlock(event.getPokemon1(), p1, p2) ||
                attemptBlock(event.getPokemon2(), p2, p1)) {
    ...
    private boolean attemptBlock(Pokemon pokemon, ServerPlayer owner, ServerPlayer partner) {
        ...
        if (ownerTooHigh || partnerTooHigh) {
            if (debug) {
                Logger.debug(String.format(
                        "%s (cap %d) versuchte %s (lvl %d) zu handeln an %s (cap %d) – geblockt",
                        ownerName, ownerCap, species, level, partnerName, partnerCap
                ));
            }

            partner.sendSystemMessage(
                    Component.literal(ConfigLoader.getMessagesConfig().getTradeBlocked())
            );
            owner.sendSystemMessage(
                    Component.literal(ConfigLoader.getMessagesConfig().getTradeBlockedPartner())
            );
        ...
        if (debug) {
            Logger.debug(String.format(
                    "%s (cap %d) hat %s (lvl %d) erfolgreich an %s (cap %d) gehandelt",
                    ownerName, ownerCap, species, level, partnerName, partnerCap
            ));
        }
        return false;
